    ...
    <>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[150] bg-ink flex flex-col pt-24 px-8 md:hidden">
          {[...NAV_LINKS, { to: '/search', label: 'Search' }, { to: '/wishlist', label: 'Wishlist' }, { to: '/cart', label: cartCount > 0 ? `Cart (${cartCount})` : 'Cart' }, { to: user ? '/account' : '/login', label: user ? `Hi, ${user.name.split(' ')[0]}` : 'Account' }].map((l) => (
            <NavLink
              key={l.to + l.label}
              to={l.to}
              onClick={closeMenu}
